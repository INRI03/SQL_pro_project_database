...
--Запросы на модификацию данных из удаленной базы
...
-- Здесь были проблемы, я не стал писать в слаке, потому что не наглядно. При первом запросе на модификацию в удаленную remote_users
-- в таблице users из сервера remote_flowers, которую она дублирует чтоб вносить туда данные, длина поля не изменилась. Следовательно
-- запрос не работает. По второму запросу на модификацию в удаленную remote_orders тоже не срабатывает ничего. В свойствах самой таблицы 
-- remote_orders появляется поле order_date, а при переходе на вкладку "данные" пишет "ОШИБКА: столбец "order_date" не существует."
-- Ровно как и в таблице orders удаленного сервера remote_flowers тоже ничего не появляется. Что я делаю не так? Скрины приложил.
